...
from ...models.role import Role as RoleSchema, RoleCreate, RoleUpdate
from ...models.person import Person as PersonSchema # Import Person schema for current_user
from ...services.role_service import role_service
from ..deps import get_db, get_current_active_user, allow_people_read, RequirePermission, RolePermissions # Import new permission tools
...
def create_role(
    role_in: RoleCreate,
    db: Session = Depends(get_db),
    current_user: PersonSchema = Depends(RequirePermission(RolePermissions.CREATE))
):
    """
    Create a new role.
...
def update_role(
    ...
    role_in: RoleUpdate,
    db: Session = Depends(get_db),
    current_user: PersonSchema = Depends(RequirePermission(RolePermissions.UPDATE))
):
    """
    Update an existing role.
    (Permissions for this action to be defined)
    """
    updated_role = role_service.update_role(db=db, role_id=role_id, role_in=role_in)
    if not updated_role:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Role not found")
